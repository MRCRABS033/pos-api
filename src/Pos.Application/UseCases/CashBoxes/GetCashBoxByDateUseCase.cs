...
public class GetCashBoxByDateUseCase
{
    private readonly ICashBoxRepository _cashBoxRepository;

    public GetCashBoxByDateUseCase(ICashBoxRepository cashBoxRepository)
    {
        _cashBoxRepository = cashBoxRepository;
    }

    public async Task<IReadOnlyList<CashBoxResponseDto>> ExecuteAsync(DateTime date)
    {
        var start = DateTime.SpecifyKind(date.Date, DateTimeKind.Utc);
        var end = start.AddDays(1);
        var cashBoxes = await _cashBoxRepository.GetByDateRangeAsync(start, end);
